...
class ColumnMapper(tk.Frame):
    ...
    def load_file(self, filepath: str):
        ...
        try:
            # Read only the header and first 10 rows of actual data
            df_preview = pd.read_excel(filepath, nrows=10)
            self.file_columns = list(df_preview.columns)

            self._update_data_preview(df_preview)
            self._update_mapping_dropdowns()
    ...
    def _create_widgets(self):
        ...
        for i, (field_key, field_label) in enumerate(self.REQUIRED_FIELDS.items()):
            ttk.Label(controls_frame, text=field_label).grid(row=i, column=0, sticky="w",
                                                             padx=5, pady=2)
        ...
        preset_frame.grid(row=len(self.REQUIRED_FIELDS), column=0, columnspan=2,
                          pady=10, sticky="ew")

        self.save_button = ttk.Button(preset_frame, text="Save Mapping...",
                                      command=self._save_mapping)
        self.save_button.pack(side="left", padx=5)
        self.load_button = ttk.Button(preset_frame, text="Load Mapping...",
    ...
    def _save_mapping(self):
        ...
        preset_name = filedialog.asksaveasfilename(
            initialdir=MAPPING_PRESETS_DIR,
...
if __name__ == '__main__':
    def handle_mapping_update(updated_mapping: ColumnMapping):
        """Dummy callback for example usage."""
        print("Mapping updated:", updated_mapping)

    # Create a dummy excel file for testing
    if not os.path.exists("data"):
        os.makedirs("data")
    DUMMY_FILEPATH = "data/dummy_for_test.xlsx"
    DUMMY_DATA = {
        "Transaction Description": ["Walmart", "Amazon", "Starbucks"],
        "Location": ["New York", "Seattle", "Online"],
        "Country Code": ["USA", "USA", "USA"],
    }
    pd.DataFrame(DUMMY_DATA).to_excel(DUMMY_FILEPATH, index=False)

    app_root = tk.Tk()
    app_root.title("Column Mapper Example")
    app_root.geometry("800x600")

    mapper_frame = ColumnMapper(app_root, on_mapping_update=handle_mapping_update,
                                padx=10, pady=10)
    mapper_frame.pack(fill="both", expand=True)
    mapper_frame.load_file(DUMMY_FILEPATH)
