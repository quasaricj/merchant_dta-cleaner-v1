...
class ConfirmationScreen(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        for i, (key, value) in enumerate(summary_data.items()):
            ttk.Label(summary_frame, text=f"{key}:", font=("Arial", 10, "bold")).grid(row=i, column=0, sticky="w", pady=2, padx=5)
            ttk.Label(summary_frame, text=value, wraplength=350, anchor="w").grid(row=i, column=1, sticky="ew", pady=2, padx=5)
        ...
        for i, (key, value) in enumerate(mappings.items()):
            ...
            ttk.Label(mapping_frame, text=f"{label}:", font=("Arial", 10, "bold")).grid(row=i, column=0, sticky="w", pady=2, padx=5)
            ttk.Label(mapping_frame, text=value if value else "Not Mapped", foreground="gray" if not value else "black").grid(row=i, column=1, sticky="ew", pady=2, padx=5)

        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill="x", pady=(15, 0))

        self.confirm_button = ttk.Button(button_frame, text="Confirm and Start", command=self._on_confirm)
