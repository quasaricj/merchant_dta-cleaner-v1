...
class MainWindow(tk.Tk):
    ...
    def _create_config_widgets(self, parent):
        ...
        self.file_selector = FileSelector(file_selector_frame, on_file_select=self.handle_file_selection)
    ...
    def _on_model_select(self, event=None):
        if self.job_settings: self.job_settings.model_name = self.model_selector.get()
        self.validate_for_processing()

    def handle_file_selection(self, filepath: str):
        self.reset_ui_for_new_file()
        self.job_settings = JobSettings(input_filepath=filepath, output_filepath=self._generate_output_path(filepath), column_mapping=ColumnMapping(merchant_name="<not_mapped>"), start_row=2, end_row=-1, mode="Basic")
    ...
    def update_cost_estimate(self):
        if not self.job_settings or self.job_settings.end_row < self.job_settings.start_row: self.cost_label.config(text="Estimated Cost: ₹0.00", foreground="black"); return
    ...
    def validate_for_processing(self):
        is_ready = self.api_keys_validated and self.job_settings and self.job_settings.column_mapping.merchant_name and self.job_settings.column_mapping.merchant_name != "<not_mapped>" and self.job_settings.model_name
        tooltip_text = ""
    ...
    def _generate_output_path(self, input_path: str) -> str:
        directory, filename = os.path.split(input_path); name, ext = os.path.splitext(filename)
    ...
    def _validate_api_and_load_models(self):
        ...
        if models:
            ...
            self.model_selector.config(values=self.available_models, state="readonly")
            if self.available_models: self.model_selector.set(self.available_models[0])
            if self.job_settings: self.job_settings.model_name = self.model_selector.get()
    ...
    def _run_diagnostics(self):
        ...
        if not self.api_config.is_valid():
            ...
            report += "   ➡️ Fix: Go to Settings > API Keys and ensure all required fields are filled.\n\n"
        else:
            report += "✅ API Keys: All required keys are present in the configuration.\n\n"
            # 2. Check Gemini API Connectivity
            ...
            if models:
                ...
                report += "❌ Gemini API Key: Validation failed. The key is likely incorrect or has expired.\n"
                report += "   ➡️ Fix: Go to Settings > API Keys and enter a valid Gemini API Key.\n\n"

        # 3. Check File Paths and Permissions
        if self.job_settings:
            input_path = self.job_settings.input_filepath
            output_path = self.job_settings.output_filepath
            output_dir = os.path.dirname(output_path)

            report += f"ℹ️ Input File: {input_path}\n"
            if not os.path.exists(input_path):
                report += "❌ Input File: The selected input file does not exist.\n"
                report += "   ➡️ Fix: Please select a valid input file.\n\n"
            elif not os.access(input_path, os.R_OK):
                report += "❌ Input File: Cannot read the selected input file. Check permissions.\n\n"
            else:
                report += "✅ Input File: File exists and is readable.\n\n"

            report += f"ℹ️ Output Directory: {output_dir}\n"
            if not os.path.isdir(output_dir):
                report += "❌ Output Directory: The directory for the output file does not exist.\n\n"
            elif not os.access(output_dir, os.W_OK):
                report += "❌ Output Directory: Cannot write to the output directory. Check permissions.\n\n"
            else:
                report += "✅ Output Directory: Directory exists and is writable.\n\n"

        else:
            report += "ℹ️ File Paths: No job has been configured, skipping file path checks.\n"
