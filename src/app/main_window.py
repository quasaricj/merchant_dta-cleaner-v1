...
class MainWindow(ttk.Frame):
    """The main application window, now a Frame for better testability."""
    ...
    def _create_logo_scraper_tab(self, parent_tab):
        ...
        file_selector_frame.pack(fill="x", pady=5)
        # The logo-only scraper doesn't have a separate output file, so we provide a dummy lambda for on_output_select.
        self.logo_only_file_selector = FileSelector(
            file_selector_frame,
            on_file_select=self._handle_logo_file_selection,
            on_output_select=lambda: None,
    ...
    def _handle_logo_mapping_update(self, mapping: Optional[ColumnMapping]):
        ...
        # This function is just a placeholder to satisfy the ColumnMapper.
        # The validation function will pull the mapping directly.
        self._validate_logo_scraper_readiness()
    ...
    def start_processing(self):
        ...
        except (ConnectionError, RuntimeError, PermissionError, FileNotFoundError) as e:
            self.logger.error(f"Failed to start job: {e}", exc_info=True)
            messagebox.showerror("Failed to Start Job", f"Could not start the job due to an error:\n\n{e}\n\nPlease check your settings, file permissions, and internet connection.")
    ...
    def pause_job(self):
        ...
        # No pause for logo-only, it runs to completion or is stopped.

    def resume_job(self):
        if self.job_manager: self.job_manager.resume(); self.progress_monitor.job_resumed()
        # No resume for logo-only

    def stop_job(self):
        # Use is_alive() to prevent errors if the thread is already finished
        if self.job_manager and self.job_manager._thread and self.job_manager._thread.is_alive() and messagebox.askyesno("Confirm Stop", "Are you sure you want to stop the enrichment job? Progress will be saved."):
            self.job_manager.stop()
        if self.logo_only_job_manager and self.logo_only_job_manager._thread and self.logo_only_job_manager._thread.is_alive() and messagebox.askyesno("Confirm Stop", "Are you sure you want to stop the logo scraping job?"):
    ...
    def handle_logo_completion(self, final_status: str):
        # This is now only called from the main JobManager after enrichment
    ...
    def _finalize_logo_scraping_ui(self, final_status: str):
        ...
        self._show_logo_completion_dialog(final_status, logo_dir_path)
        self.job_manager = None # Fully release the enrichment manager
    ...
    def toggle_config_widgets(self, enabled: bool):
        ...
        """
        # Also disable/enable the logo scraper tab
        self.notebook.tab(1, state="normal" if enabled else "disabled")

        state = "normal" if enabled else "disabled"
        # Explicitly toggle each major component in the enrichment tab
        ...
        self.validate_for_processing() # This will correctly set the button state
    ...
    def _start_api_validation_thread(self):
        ...
        self.progress_monitor.status_label.config(text="Status: Validating API keys...")
...
class Application(tk.Tk):
    """The root Tkinter application which hosts the MainWindow frame."""
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            logging.getLogger(__name__).warning("Application icon 'j_cleans_icon.ico' not found. Skipping.")
    ...
    def create_menu(self):
        ...
        settings_menu.add_command(label="API Keys", command=self.main_window.open_api_key_dialog)

        help_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Help", menu=help_menu)
        help_menu.add_command(label="View User Guide", command=self.main_window.show_user_guide)
        help_menu.add_separator()
        help_menu.add_command(label="Contact Support", command=self.main_window.show_contact_info)
