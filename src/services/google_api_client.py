...
class GoogleApiClient:
    ...
    def validate_and_enrich_from_search(self, search_results: List[Dict], cleaned_name: str, query: str, detected_aggregator: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        # Dynamically add aggregator context to the prompt if one was detected
        aggregator_context = ""
        if detected_aggregator:
            aggregator_context = f"- A payment aggregator ('{detected_aggregator}') was detected and removed from the original string. Your first step in the evidence should be to state this."
