...
from src.core.constants import PAYMENT_AGGREGATORS
...
class ProcessingEngine:
    ...
    def __init__(self, settings: JobSettings, api_client: GoogleApiClient):
        ...
        self.detected_aggregator = None # To store if an aggregator is found
    ...
    def _pre_process_name(self, record: MerchantRecord):
        ...
        including the removal of known payment aggregator names.
        """
        import re
        ...
        # Step 1: Detect and remove payment aggregators
        for aggregator in PAYMENT_AGGREGATORS:
            if aggregator in cleaned_name:
                self.detected_aggregator = aggregator
                # Replace aggregator and any trailing non-alphanumeric chars
                cleaned_name = re.sub(re.escape(aggregator) + r'\W*', '', cleaned_name).strip()
                break  # Stop after finding the first one

        # Step 2: Remove other common machine-generated prefixes and residual junk
        cleaned_name = re.sub(r'^\d+\s*|^\W+', '', cleaned_name)
    ...
    def _perform_basic_search(self, record: MerchantRecord, query: str) -> bool:
        ...
        ai_decision = self.api_client.validate_and_enrich_from_search(
            search_results, record.cleaned_merchant_name, query, self.detected_aggregator
        )
